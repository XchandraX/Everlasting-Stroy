...
<div class="fade-up max-w-5xl mx-auto">
    <div class="mb-10 flex justify-between items-end">
        <div>
            ...
            <h2 class="text-3xl font-black italic text-white uppercase tracking-tighter">Upload<span class="text-cyan-400">_Asset</span></h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div class="lg:col-span-7">
            <div class="bg-black/40 backdrop-blur-md border border-white/5 p-8 rounded-3xl relative overflow-hidden group">
                ...
                <form action="{{ route('admin.images.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-6">
                        <div>
                            <label class="block text-[10px] font-mono text-pink-500 uppercase tracking-[0.2em] mb-2">Asset_Title</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-[10px] font-mono text-pink-500 uppercase tracking-[0.2em] mb-2">Target_Node</label>
                                <div class="relative">
                                    <select name="kategori_id" required 
                                        ...
                                        @foreach($categories as $cat)
                                            <option value="{{ $cat->id }}" class="bg-slate-900">{{ strtoupper($cat->nama_kategori) }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div>
                                <label class="block text-[10px] font-mono text-pink-500 uppercase tracking-[0.2em] mb-2">Data_Stream</label>
                                <label for="imgUpload" class="flex items-center justify-center w-full bg-cyan-500/10 border border-cyan-500/30 rounded-xl px-4 py-3 cursor-pointer hover:bg-cyan-500/20 transition group">
                                    ...
                                    <span class="text-[10px] font-black text-cyan-500 uppercase tracking-widest">Select_File</span>
                                    <input type="file" name="image" id="imgUpload" accept="image/*" required class="hidden">
                                </label>
                            </div>
                        </div>

                        <div>
                            <label class="block text-[10px] font-mono text-pink-500 uppercase tracking-[0.2em] mb-2">Description_Log</label>
                        ...
                        <div class="pt-4 flex gap-4">
                            <button type="submit" class="flex-[2] bg-pink-600 text-white font-black py-4 rounded-xl hover:bg-pink-500 shadow-[0_0_25px_rgba(219,39,119,0.3)] transition-all active:scale-95 uppercase text-[10px] tracking-[0.3em]">
                                Transmit_Data_Stream
                            </button>
                            <a href="{{ route('admin.images.index') }}" class="flex-1 flex items-center justify-center border border-white/10 rounded-xl text-gray-500 hover:text-white hover:bg-white/5 transition uppercase text-[10px] font-bold tracking-widest">
                                Abort
                            </a>
                        </div>
                    </div>
        ...
        <div class="lg:col-span-5 h-full min-h-[400px]">
            <div class="h-full border border-white/10 rounded-3xl bg-black/20 flex flex-col items-center justify-center p-8 relative overflow-hidden group backdrop-blur-sm">
                ...
                <div id="previewContainer" class="hidden w-full h-full flex flex-col items-center justify-center z-20">
                    <div class="relative w-full aspect-square rounded-2xl overflow-hidden border border-cyan-500/30 shadow-[0_0_40px_rgba(6,182,212,0.1)]">
                        ...
                        <div class="absolute top-4 left-4 right-4 flex justify-between items-start">
                            <div class="bg-black/80 backdrop-blur-md border border-cyan-500/50 px-2 py-1 rounded">
                                <p class="text-[8px] font-mono text-cyan-400 uppercase tracking-tighter">Live_Preview_v1.0</p>
                            </div>
                    ...
                    <div class="mt-6 space-y-2 w-full">
                        ...
                        <p class="text-[9px] text-cyan-400 font-mono text-center tracking-[0.2em] uppercase">
                            [ Visual_Data_Sync_Complete ]
                        </p>
                    </div>
                </div>
                
                <div id="placeholder" class="text-center z-20 group-hover:scale-110 transition-transform duration-500">
                    <div class="w-20 h-20 mx-auto mb-6 rounded-full border border-dashed border-white/20 flex items-center justify-center">
                    ...
                    <p class="text-[9px] text-gray-700 mt-2">Recommended: 1080p+ JPG/PNG</p>
                </div>
            </div>
        </div>
    </div>
