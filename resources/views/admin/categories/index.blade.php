            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-white/5 border-b border-white/10">
                    <tr>
                        ...
                        <th class="px-6 py-5 text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">Asset Count
                        </th>
                        <th class="px-6 py-5 text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">Status</th>
                        <th class="px-6 py-5 text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] text-right">
                            Operation</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-white/5">
                    @foreach ($categories as $cat)
                        <tr class="hover:bg-cyan-500/5 transition-all group">
                            <td class="px-6 py-4">
                                <div
                                    class="w-14 h-10 rounded border border-white/10 overflow-hidden bg-slate-800 group-hover:border-cyan-500/50 transition-colors">
                                    @if ($cat->cover_image)
                                        <img src="{{ asset('storage/' . $cat->cover_image) }}"
                                            class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                    @else
                                        <div
                                            class="w-full h-full flex items-center justify-center bg-slate-900 text-[10px] text-gray-600">
                                            NO_IMG</div>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="font-bold text-white group-hover:text-cyan-400 transition-colors uppercase tracking-tight">{{ $cat->nama_kategori }}</span>
                                <p class="text-[9px] text-gray-600 font-mono mt-1 tracking-tighter">UUID:
                                    {{ substr(md5($cat->id), 0, 8) }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <span
                                        class="text-sm font-mono text-gray-300">{{ str_pad($cat->images_count, 2, '0', STR_PAD_LEFT) }}</span>
                                    <span class="ml-2 text-[9px] text-gray-600 uppercase">Records</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold bg-cyan-500/10 text-cyan-500 border border-cyan-500/20">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-3">
                                    ...
                                    <form action="{{ route('admin.categories.destroy', $cat->id) }}" method="POST"
                                        ...
                                        @csrf @method('DELETE')
                                        <button type="button"
                                            class="delete-btn bg-transparent border-none p-0 m-0 text-gray-500 hover:text-pink-500 transition-all transform hover:scale-110">
